...
gui.ResetOnSpawn = false -- Важно: GUI не исчезнет после смены локации
...
-- Переменные управления циклом
local isRunning = false
local currentLoop
local isInHiveHub = false -- Флаг, чтобы знать, где мы находимся
...
-- Функция для проверки, находимся ли мы в Hive Hub
local function checkIfInHiveHub()
    -- Ищем характерные объекты Hive Hub
    for _, v in ipairs(Workspace:GetDescendants()) do
        if v:IsA("BasePart") and (v.Name:lower():find("hive") or v.Name:lower():find("hub")) then
            return true
        end
    end
    return false
...
-- Функция для ходьбы персонажа к цели (без телепортации)
...
local function goToHiveHub()
    ...
    if portal then
        ...
        walkTo(portal.Position + Vector3.new(0, 0, 0))
        task.wait(2)
        updateStatus("Entering portal...")
        -- Встаем прямо на портал, чтобы активировать его касанием
        local root = getHumanoidRootPart()
        if root then
            root.CFrame = portal.CFrame * CFrame.new(0, 2, 0)
        end
        task.wait(5) -- Ждем загрузки Hive Hub
        isInHiveHub = true
...
local function gameLoop()
    while isRunning do
        local success, err = pcall(function()
            ...
            if isInHiveHub then
                updateStatus("Already in Hive Hub, doing quest...")
                doQuest()
            else
...
-- Функция для полной остановки и сброса
...
-- АВТОМАТИЧЕСКИЙ ЗАПУСК ПРИ ИНЖЕКТИРОВАНИИ
startLoop()

-- Следим за сменой локации (респавном), чтобы обновить флаг и продолжить работу
player.CharacterAdded:Connect(function(character)
    if isRunning then
        updateStatus("Respawning...")
        task.wait(2)
        isInHiveHub = checkIfInHiveHub()
        updateStatus("Respawned. Running...")
    end
end)
