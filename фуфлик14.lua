...
-- Поиск портала (несколько точных вариантов)
local function findPortal()
    local portalNames = {"Hive Hub Portal", "Portal_HiveHub", "HiveHubPortal", "Hive Hub"}
    for _, name in ipairs(portalNames) do
        local portal = Workspace:FindFirstChild(name, true)
...
-- Поиск NPC (несколько точных вариантов)
local function findNPC()
    local npcNames = {"Sticker-Seeker Quest Machine", "Sticker Seeker", "StickerSeeker", "Quest Machine"}
    for _, v in ipairs(Workspace:GetDescendants()) do
        if v:IsA("Model") then
            for _, name in ipairs(npcNames) do
                if v.Name == name and v:FindFirstChild("Humanoid") and v:FindFirstChild("Head") then
                    return v
                end
            end
        end
