...
btnStop.BackgroundColor3 = Color3.fromRGB(255, 105, 180) -- розовый
...
-- Проверка, находимся ли мы в Hive Hub (по наличию характерных объектов)
...
-- Ходьба с использованием PathfindingService
...
-- Поиск портала по ключевым словам (КАК В РАБОЧЕЙ ВЕРСИИ)
...
-- Поиск NPC Sticker-Seeker по ключевым словам
...
local function goToHiveHub()
    ...
    if portal then
        ...
        local root = getRoot()
        if root then
            -- Встаём на портал для активации
...
-- Обработка возрождения после смены локации
